...
namespace Jerrycurl.Mvc.Sql
{
    ...
    {
        /// <summary>
        /// Appends mappings between the current qualified columns and their properties, e.g. <c>T0."MyColumn" AS "Item.MyProperty"</c>, to the projection buffer. Identical to calling <c>Cols().As().Props()</c>.
        /// </summary>
        ...
        [Obsolete("Use .Map() extension instead.")]
        ...
        /// <summary>
        /// Appends mappings between the selected qualified columns and their properties, e.g. <c>T0."MyColumn" AS "Item.MyProperty"</c>, to the projection buffer. Identical to calling <c>Cols().As().Props()</c>.
        /// </summary>
        /// <param name="projection">The current projection</param>
        ...
        /// <param name="tblAlias">The table alias to qualify each column name with.</param>
        /// <returns>A new projection containing the appended buffer.</returns>
        public static IProjection Map(this IProjection projection, string tblAlias = null)
        {
            if (!projection.Any())
                throw ProjectionException.AttributesNotFound(projection.Metadata);
        ...
        /// <param name="tblAlias">The table alias to qualify each column name with.</param>
        /// <returns>A new projection containing the appended buffer.</returns>
        public static IProjection Map<TModel, TProperty>(this IProjection<TModel> projection, Expression<Func<TModel, TProperty>> expression, string tblAlias = null) => projection.For(expression).Map(tblAlias);
    }
}
